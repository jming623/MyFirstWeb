	...
	<div class="container">
		...
		<div>
			<select onchange="change(this)">
				<option value="10" ${pageVO.amount eq 10 ? 'selected' : '' } >10개씩보기</option>
				<option value="20" ${pageVO.amount eq 20 ? 'selected' : '' } >20개씩보기</option>
				<option value="50" ${pageVO.amount eq 50 ? 'selected' : '' } >50개씩보기</option>
				<option value="100" ${pageVO.amount eq 100 ? 'selected' : '' } >100개씩보기</option>			
			</select>		
		</div>
		...
		<table class="table table-bordered">
			...
			<c:forEach var="vo" items="${list }">
				<tr>
					...
					<td><fmt:formatDate value="${vo.regdate }" pattern="yyyy년MM월dd일 HH시mm분"/></td>
					<td>${vo.hit }</td>
				</tr>
			</c:forEach>		
